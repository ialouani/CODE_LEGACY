...
int hashAdd(struct HashTable *hash,void *data){
  ...
  //considerer les allocation seulement si necessaire
  //cas contraire : probleme de gestion des 'free'.
  /*if(hash==NULL) free(cell);*/
  assert(hash!=NULL);
...
void hashFree(struct HashTable *hash, void (*release)(void *)){
  ...
  for(i=0;i<hash->size;++i){
    ...
    //p->next == NULL donc p est la derniere cellule en boucle courante
    //modele des listes chainees.
    free(p);
  }
  free(hash->entry);
  free(hash);    
